    ...
    if (!parentId) {
      ...
      if (postOwner && postOwner.id !== user.id) {
        ...
        await sendCommentNotification({
          to: postOwner.email,
          postAuthorName: postOwner.displayName,
      ...
      if (parentId !== parentComment.rootId) {
        return res.status(400).json({
          success: false,
      ...
      if (parentComment.user.id !== user.id) {
        ...
        await sendReplyNotification({
          ...
          commentAuthorName: parentComment.user.displayName,
...
export const deleteComment = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      ...
      error: "ServerError",
      message:
        "Unable to delete comment due to a server error. Please try again.",
    });
  }
};
